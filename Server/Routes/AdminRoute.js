...
router.get('/filter/:value/:minsalary/:maxsalary', async(req, res) => {
    const name = req.params.value;
    const maxsalary = req.params.maxsalary;
    const minsalary = req.params.minsalary;
    const sql = "SELECT * FROM employee WHERE name=? OR (salary>=? AND salary<=?)";
    con.query(sql, [name, minsalary, maxsalary], (err, result) => {
        if (err) {
